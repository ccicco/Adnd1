...
// Rebuild tranche R19: monsters live in rooms. Proximity triggers.
//
//   - Rooms populated at generation (~half occupied, level-tagged)
//   - Occupied rooms tinted; monsters drawn as team-colored markers
//   - Entering an occupied room starts the fight; cleared after
//   - Wandering monsters still roam (unchanged from R17)
//
// Build (MinGW, Lua 5.4):
...
// ----------------------------------------------------------------------------
// Room occupancy: which room a tile is in, and who lives there
// ----------------------------------------------------------------------------
...
struct RoomOccupant {
    ...
    int         count = 0;
    // den position (where the markers are drawn)
    int         denX = 0, denY = 0;
};

struct Occupancy {
    // room index -> occupant; empty = unoccupied
...
struct Party {
    int x = 0, y = 0;
    // The test party: one fighter for now (roster grows with the
    // character-creation tranche).
    ai::Actor fighter;
    bool formed = false;
    ...
    void formDefault() {
        ...
        fighter.shield = true;
        fighter.hp = fighter.maxHp = 9;   // 1d10 with con adj, average
...
struct AppState {
    ...
    // Populate ~half the rooms with level-appropriate monsters.
    void populateRooms() {
        ...
        for (auto& room : occupancy.rooms) {
            ...
            // 1-3, small rooms fewer
            int maxCount = roomCountCap();
            room.count = 1 + (int)rng.below((uint32_t)maxCount);
        }
    }
    ...
    // Which room is a tile in? -1 if none (corridor/void).
    ...
    // Proximity check: is the party in or next to an occupied room?
    ...
    // Spawn the occupants of a room; clears the occupancy.
    void spawnRoomEncounter(int roomIndex) {
        ...
        char buf[96];
        const MonsterName mn = monsterDisplayName(room.monsterKey);
        snprintf(buf, sizeof buf, "%d %s%s leap at you!",
                 room.count, mn.plural, room.count > 1 ? "" : "");
        if (room.count == 1)
            snprintf(buf, sizeof buf, "A %s leaps at you!", mn.singular);
        log.add(buf);

        combat.start({party.fighter}, foes, rng.below(0x7FFFFFFF));
        combatRoomIndex = roomIndex;   // cleared on victory
        mode = MODE_COMBAT;
    }

    struct MonsterName { const char* singular; const char* plural; };
    MonsterName monsterDisplayName(const std::string& key) const {
        const monsters::MonsterDef* d = registry.find(key);
        if (d) return { d->name.c_str(), d->name.c_str() };
        return { "monster", "monsters" };
    }

    // Spawn a wandering encounter (unchanged behavior).
    void spawnWanderingEncounter() {
        ...
        combatRoomIndex = -1;   // not a room fight
        combat.start({party.fighter}, foes, rng.below(0x7FFFFFFF));
        mode = MODE_COMBAT;
    }

    // Combat bookkeeping: which room the fight came from (-1 wander)
    ...
    void endCombat() {
        if (combat.encounter) {
            ...
            // Room cleared on a party win (monsters dead or routed)
...
// occupied-room tint: a warm red wash over floor tiles
static void drawOccupancyTint(HDC dc, int px, int py) {
    HBRUSH br = CreateSolidBrush(RGB(90, 30, 25));
    RECT r = { px, py, px + TILE_W, py + TILE_H };
    // 30% alpha-ish: use a hatch instead (GDI basic)
...
static void drawMonsterMarker(HDC dc, int px, int py, int count) {
    // team-colored marker with a count pip
...
static void drawCombatantRow(HDC dc, int x, int y, int w,
    ...
    char line[96];
    snprintf(line, sizeof line, "%s", a.name.c_str());
...
static void drawCombat(HDC dc, const CombatState& cs) {
    ...
    char line[96];
    snprintf(line, sizeof line, "COMBAT!  [space] next round   [esc] auto-resolve");
    ...
    for (const auto& a : party) {
        drawCombatantRow(dc, 20, y, VIEW_W / 2 - 40, a, true);
        y += 26;
    }
    ...
    y = 70;
    for (const auto& a : mons) {
        drawCombatantRow(dc, VIEW_W / 2 + 20, y, VIEW_W / 2 - 40, a, false);
        y += 26;
    }
        ...
        case WM_KEYDOWN:
            switch (wp) {
                ...
                case 'N':
                    ...
                    g_app.mode = MODE_EXPLORE;
                    break;

                case 'E':
                    if (g_app.mode == MODE_EXPLORE && g_app.party.alive())
                        g_app.spawnWanderingEncounter();
                    break;

                case 'F': {  // dev: force a wight in the nearest room
                    if (g_app.mode != MODE_EXPLORE || !g_app.party.alive())
                        break;
                    // occupy the nearest unoccupied room with a wight
                    int best = -1;
                    long bestDist = -1;
                ...
                case VK_SPACE:
                    if (g_app.mode == MODE_COMBAT) {
                        bool ended = g_app.combat.step();
                        if (ended) g_app.endCombat();
                    }
                    break;
