...
// Rebuild tranche R17: encounters come alive. EXPLORE <-> COMBAT.
//
//   - Win32 window + message pump, GDI double-buffered rendering
//   - world/map.h shared tile map, dm/dungeon.cpp Appendix A generator
//   - monsters/MonsterRegistry: Lua bestiary
//   - Wandering checks spawn real fights (ai::Encounter, R15 driver)
//   - Combat view: both sides, hp, log; SPACE steps a round
//   - Damage persists after the fight
//
// Build (MinGW, Lua 5.4):
...
// ----------------------------------------------------------------------------
// Combat state: a live encounter driven round by round
// ----------------------------------------------------------------------------

struct CombatState {
    std::unique_ptr<ai::Encounter> encounter;
    int  lastResult = -1;   // -1 ongoing; 0/1/2/3 per Encounter
    bool over = false;
    ...
    // step one round; returns true if the fight ended
    bool step() {
...
struct AppState {
    ...
    void newDungeon(uint64_t s) {
        ...
        cam.follow(party);
        turnCount = 0;
        rng.seed(s * 7919 + 13);   // fresh stream per dungeon
        char buf[96];
    ...
    // Spawn a wandering encounter and switch to combat mode.
    void spawnEncounter(const std::string& forcedKey = "") {
        if (mode == MODE_COMBAT) return;
        if (!party.alive()) return;

        std::string key = forcedKey;
        if (key.empty()) {
            auto candidates = registry.keysForLevel(dungeonLevel);
            if (candidates.empty()) return;
            key = candidates[(size_t)rng.below((uint32_t)candidates.size())];
        }

        // 1-3 monsters (1-2 at dungeon level 1)
        int count = 1 + (int)rng.below(
            dungeonLevel >= 2 ? 3 : 2);
        ...
        char buf[96];
        snprintf(buf, sizeof buf, "%d %s%s attack!",
                 count, foes[0].name.c_str(), count > 1 ? "s" : "");
        log.add(buf);
    ...
    // Return from combat to exploration; carry damage over.
    void endCombat() {
        if (combat.encounter) {
            // party hp persists: copy back from the encounter
...
// ----------------------------------------------------------------------------
// Drawing: combat view — two rosters, hp bars, log tail
// ----------------------------------------------------------------------------
...
static void drawHud(HDC dc, const Party& party, int turnCount,
                    uint64_t seed, int roomCount, int dungeonLevel,
    ...
    snprintf(line, sizeof line,
             "Rolf HP %d/%d   Turn: %d   Seed: %llu   Rooms: %d   Lvl %d",
             party.fighter.hp, party.fighter.maxHp, turnCount,
             (unsigned long long)seed, roomCount, dungeonLevel);
    TextOutA(dc, 12, VIEW_H + 8, line, (int)strlen(line));

    snprintf(line, sizeof line,
             "[arrows/WASD] move  [N] new dungeon  [E] encounter  "
    ...
    switch (msg) {
        ...
        case WM_KEYDOWN:
            switch (wp) {
                ...
                case 'E':   // dev: force an encounter
                    if (g_app.mode == MODE_EXPLORE && g_app.party.alive())
                        g_app.spawnEncounter();
                    break;

                case 'F':   // dev: force a wight (gating test)
                    if (g_app.mode == MODE_EXPLORE && g_app.party.alive())
                        g_app.spawnEncounter("wight");
                    break;

                case VK_SPACE:
                    // combat: step one round
                ...
                case VK_ESCAPE:
                    if (g_app.mode == MODE_COMBAT) {
                        // esc during combat: auto-resolve the rest
        ...
        case WM_PAINT: {
            ...
            if (g_app.rend.memDC) {
                if (g_app.mode == MODE_COMBAT) {
                    ...
                    drawView(g_app.rend.memDC, g_app.map, g_app.cam,
                             g_app.party);
                }
                drawHud(g_app.rend.memDC, g_app.party, g_app.turnCount,
                        g_app.seed, g_app.roomCount, g_app.dungeonLevel,
                        g_app.log);
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, LPWSTR, int nCmdShow) {
    ...
    if (monstersLoaded <= 0) {
        ...
        // game still runs; encounters just won't spawn
    }
